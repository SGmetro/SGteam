...
SG Macro is free software: you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation,
either version 3 of the License, or (at your option) any later version.

SG Macro is distributed in the hope that it will be useful. This does not give you the right to steal sections from our code, distribute it under your own name, then slander the macro.

You should have received a copy of the license along with SG Macro. If not, please redownload from an official source.
*/

;Compiler directives (currently not in use):
;@Ahk2Exe-SetName SG Macro
;@Ahk2Exe-SetDescription SG Macro
;@Ahk2Exe-SetCompanyName SG Team
;@Ahk2Exe-SetCopyright Copyright © SG Team
;@Ahk2Exe-SetOrigFilename sg_macro.exe
#MaxThreads 255
...
ElevateScript() {
	try
		...
		if !A_IsAdmin {
			MsgBox "You must run SG Macro as administrator in this folder!`nIf you don't want to do this, move the macro to a different folder (e.g. Downloads, Desktop)", "Error", 0x40010
			ExitApp
		}
		; elevated but still can't write, read-only directory?
		MsgBox "You cannot run SG Macro in this folder!`nTry moving the macro to a different folder (e.g. Downloads, Desktop)", "Error", 0x40010
	}
	else
		file.Close()
}
...
nm_AutoUpdateGUI(*)
{
	...
	GuiClose()
	UpdateGui := Gui("+Border +Owner" MainGui.Hwnd " -MinimizeBox", "SG Macro Update")
	UpdateGui.OnEvent("Close", GuiClose), UpdateGui.OnEvent("Escape", GuiClose)
	UpdateGui.SetFont("s9 cDefault Norm", "Tahoma")
	UpdateText := UpdateGui.Add("Text", "x20 w260 +Center +BackgroundTrans", "A newer version of SG Macro was found!`nDo you want to update now?")

	posW := TextExtent("SG Macro v" VersionID " ⮕ v" LatestVer, UpdateText)
	UpdateGui.Add("Text", "x" 149-posW//2 " y40 +BackgroundTrans", "SG Macro v" VersionID " ⮕ ")
...
nm_MajorUpdateHelp(*)
{
	...
	. "However, in SG Macro, your Settings are guaranteed to be transferable to any new version, so that option remains enabled.`n`n"
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
OnExit(GetOut)
MainGui := Gui((AlwaysOnTop ? "+AlwaysOnTop " : "") "+Border +OwnDialogs", "SG Macro (Loading 0%)")
WinSetTransparent 255-floor(GuiTransparency*2.55), MainGui
MainGui.Show("x" GuiX " y" GuiY " w490 h275")
SetLoadingProgress(percent) => MainGui.Title := "SG Macro (Loading " Round(percent) "%)"
...
(GuiCtrl := MainGui.Add("CheckBox", "x176 yp+24 w88 h15 vReconnectMessage Disabled Checked" ReconnectMessage, "SG Natro so broke")).Section := "Settings", GuiCtrl.OnEvent("Click", nm_saveConfig)
...
;tab (un)lock
nm_LockTabs(lock:=1){
	static tabs := ["Gather","Collect","Boost","Quests","Planters","Status","Settings","Misc","Help"]
	global bitmaps
...
nm_NatroSoBrokeHelp(*){ ; so broke information
	...
	(
	DESCRIPTION:
	Enable this to have the macro say 'SG Macro so broke :weary:' in chat after it reconnects! This is a reference to e_lol's macros which type 'e_lol so pro :weary:' in chat.
	)", "SG Macro so broke :weary:", 0x40000
}
...
nm_AutoStartManager(*){
	...
	if A_IsAdmin
		MsgBox "
		(
		SG Macro has been run as administrator!
		Auto-Start Manager can only launch SG Macro on logon without admin privileges.

		If you need to run SG Macro as admin, either:
		- fix the reason why admin is required (reinstall Roblox unelevated, move SG Macro folder)
	...
	if !(task := RegRead("HKCU\Software\Microsoft\Windows\CurrentVersion\Run", "SGMacro", ""))
	...
	ASMGui.Add("Text", "x0 y24 w" w " h36 vStatusText +Center c" ((status > 0) ? "Red" : "Green")
		, ((status = 0) ? "SG Macro will automatically start on user login using the settings below:"
		: (status = 1) ? "No SG Macro auto-start found!`nUse the 'Add' button below."
		: "Your auto-start needs updating!`nUse 'Add' to create a new auto-start."))

	ASMGui.Add("Text", "x0 yp+34 vNTLabel", "SG Macro Path: ")
...
AddButton(*)
{
	...
	if (task := RegRead("HKCU\Software\Microsoft\Windows\CurrentVersion\Run", "SGmacro", ""))
		if (MsgBox("Are you sure?`nThis will overwrite the existing SG Macro auto-start!", "Overwrite Existing Entry", 0x40024 " T30 Owner" ASMGui.Hwnd) != "Yes")
			return
	...
	CenterText(ASMGui["StatusLabel"], ASMGui["StatusVal"], ASMGui["StatusLabel"])
	ASMGui["StatusText"].SetFont("cGreen"), ASMGui["StatusText"].Text := "SG Macro will automatically start on user login using the settings below:"
...
RemoveButton(*)
{
	...
	else
	{
		...
		CenterText(ASMGui["StatusLabel"], ASMGui["StatusVal"], ASMGui["StatusLabel"])
		ASMGui["StatusText"].SetFont("cRed"), ASMGui["StatusText"].Text := "No SG Macro auto-start found!`nUse the 'Add' button below."
...
nm_ContributorsImage(page:=1, contributors:=""){
	...
	if !IsSet(hBM1)
	{
		devs := [["SGmetro",0xffa202c0,"1115885772608897086"]
			, ["Siniy",0xffe6a157,"1356364425602273420"]]

		testers := [["Kitoyoma",0xffff00ff,"921611063051550740"]
			, ["e_lol",0xffa45ee9,"532593643379359748"]
			, ["SGmetro",0xffa202c0,"1115885772608897086"]
			, ["Siniy",0xffe6a157,"1356364425602273420"]]
...
start(*){
	...
	try PostMessage 0x100, 0x7, 0, , "ahk_id " (hRoblox := GetRobloxHWND())
		...
		You must either reinstall Roblox without administrative rights, or run SG Macro as admin!
